...
# Активируется при наличии GROQ_API_KEY в env.
# Бесплатно через Groq (бесплатный tier: достаточно для нашего объёма).
...
GROQ_TIMEOUT_S      = 12  # быстрый таймаут чтобы юзер не ждал
...
# System prompt — компактный, чтобы экономить токены
...
def ai_faq_answer(question: str, user_first_name: str | None = None) -> str | None:
    ...
    if not GROQ_ENABLED:
        return None
    try:
        # Адаптивный prompt — добавляем имя пользователя если есть
...
def is_ai_rate_limited(chat_id: int) -> bool:
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if step is None:
        # Если короткое сообщение или это меню-кнопка — просто показываем меню
        ...
        if is_menu_button or is_too_short:
            await update.message.reply_text(
        ...
        # ── AI-FAQ: пользователь задал свободный вопрос ──
        if not GROQ_ENABLED:
            await update.message.reply_text(
                "Не понял запрос 🙈\n\nВыберите действие из меню или напишите менеджеру:",
                reply_markup=MAIN_MENU
            )
            return
        ...
        if not answer:
            # Fallback если AI недоступен — направляем к менеджеру
            await update.message.reply_text(
        ...
        # Логируем вопрос для будущей FAQ-базы
        try:
        ...
        # Ответ + всегда добавляем CTA-кнопки
        await update.message.reply_text(
            answer,
            reply_markup=InlineKeyboardMarkup([
                [InlineKeyboardButton("🚗 Оставить заявку", callback_data="menu_apply")],
